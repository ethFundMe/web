...
import { type FeaturedCampaign } from '@/lib/types';
...
export default function FeaturedCampaign({
  ...
  const images = useMemo(
    () => [campaign.campaign_banner, ...campaign.campaign_mediaLinks],
    [campaign]
  );

  const AnimImage = motion(Image);

  const percentage = (campaign.total_accrued / campaign.campaign_goal) * 100;
      ...
      >
        ...
        <div className='mt-4 flex w-full  max-w-4xl items-center gap-x-4'>
          ...
          <div className='flex items-center font-semibold text-white'>
            <FaEthereum size={20} />
            <span className='text-xl lg:text-2xl'>
              {formatEther(BigInt(campaign.campaign_goal))}
            </span>
          </div>
        </div>
