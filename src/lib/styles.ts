export const TextSizeStyles = {
  header1: 'text-[100px] leading-[140%] font-bold',
  header2: 'text-[72px] leading-[140%] font-bold',
  h1: 'text-[36px] md:text-[52px] leading-[140%] font-bold',
  h2: 'text-34px md:text-[48px] leading-[140%] font-bold',
  h3: 'text-32px md:text-[32px] leading-[140%] font-bold',
  h4: 'text-28px md:text-[28px] leading-[140%] font-bold',
  h5: 'text-[24px] md:text-[24px] leading-[140%] font-bold',
  h6: 'text-[20px] leading-[140%] font-bold',
  body: 'text-base',
  caption: 'text-sm leading-[150%]',
  small: 'text-xs leading-[150%]',
};
